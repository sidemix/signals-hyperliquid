...
if not any(isinstance(h, logging.StreamHandler) for h in log.handlers):
    _h = logging.StreamHandler(sys.stdout); _h.setFormatter(logging.Formatter("%(levelname)s:%(name)s:%(message)s")); log.addHandler(_h)
log.propagate = False
...
def _api_url()->str: return _API_URL or constants.MAINNET_API_URL

@dataclass
class ExecPlan:
    side:str; coin:str; limit_px:float; size:float; tif:Optional[str]; reduce_only:bool=False

def _require_signer():
    if not _PRIVKEY: raise RuntimeError("Set HYPER_PRIVATE_KEY")
    if not _ACCOUNT: raise RuntimeError("Set HYPER_ACCOUNT_ADDRESS")
    try: return Account.from_key(_PRIVKEY)
    except Exception as e: raise RuntimeError(f"Invalid HYPER_PRIVATE_KEY: {e}")

def _mk_clients()->tuple[Exchange,Info]:
    signer=_require_signer(); url=_api_url()
    return Exchange(signer,url,account_address=_ACCOUNT), Info(url, skip_ws=True)

def _coin_from_symbol(symbol:str)->str: return (symbol or "").split("/")[0].upper()
def _symbol_ok(symbol:str)->bool:
    if not _ALLOWED: return True
    sym=(symbol or "").upper(); coin=_coin_from_symbol(symbol)
...
def _parse_overrides(envvar:str)->dict:
    raw=(os.getenv(envvar,"") or "").strip(); out={}
    if not raw: return out
    for item in raw.split(","):
        if "=" in item:
            k,v=item.split("=",1); k=k.strip().upper(); v=v.strip()
            try: out[k]=float(v)
            except: pass
    return out
...
def _try_get_assets_container(info:Info)->Optional[Any]:
    ...
    if isinstance(cont,(list,tuple)) and cont and isinstance(cont[0],dict): return cont
    ...
    try:
        for v in info.__dict__.values():
            if isinstance(v,(list,tuple)) and v and isinstance(v[0],dict) and ("pxDecimals" in v[0] or "px_decimals" in v[0]): return v
    except: pass
    return None
...
def _resolve_asset_dict(info:Info, coin:str)->Optional[dict]:
    try:
        ...
        if isinstance(res,int):
            ...
            if c and 0<=res<len(c): return c[res]
    except: pass
    c=_try_get_assets_container(info)
    if isinstance(c,(list,tuple)):
        for d in c:
            nm=d.get("name") or d.get("token") or d.get("symbol")
            if isinstance(nm,str) and nm.upper()==coin.upper(): return d
    return None

def _get_asset_meta(info:Info, coin:str)->tuple[float,float,Optional[float]]:
    price_tick=_FALLBACK_PRICE_TICK; size_step=_FALLBACK_SIZE_STEP; min_sz=_FALLBACK_MIN_SIZE
    ...
    if asset:
        ...
        for k in ("minSz","minSize","min_size"):
            if k in asset:
                try: min_sz=float(asset[k])
                except: pass; break
    cu=coin.upper()
    if cu in _SIZE_OVR: size_step=_SIZE_OVR[cu]
    if cu in _TICK_OVR: price_tick=_TICK_OVR[cu]
    log.info("[HL] META coin=%s price_tick=%g size_step=%g min_sz=%s", coin, price_tick, size_step, "None" if min_sz is None else f"{min_sz}")
...
def _redis_claim(client_id:str)->Optional[bool]:
    ...
    try:
        ...
        if ok: log.info("[HL] IDEMP[redis]: claimed %s", client_id); return True
        log.info("[HL] SKIP duplicate (redis) %s", client_id); return False
    except Exception as e:
        log.exception("[HL] Redis idempotency error (falling back): %s", e); return None
...
def _sqlite_conn():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS sent_client_ids (client_id TEXT PRIMARY KEY, ts INTEGER NOT NULL)")
    return conn
...
def _sqlite_claim(client_id:str)->bool:
    ...
    try:
        ...
        log.info("[HL] IDEMP[sqlite]: claimed %s", client_id); return True
    except sqlite3.IntegrityError:
        log.info("[HL] SKIP duplicate (sqlite) %s", client_id); return False
    except Exception as e:
        log.exception("[HL] SQLite idempotency error (best-effort continue): %s", e); return True
    finally:
...
def _claim_client_id(client_id:Optional[str])->bool:
    if not client_id: return True
    if client_id in _SENT_CLIENT_IDS:
        log.info("[HL] SKIP duplicate (process) %s", client_id); return False
    r=_redis_claim(client_id)
    if r is True: _SENT_CLIENT_IDS.add(client_id); return True
    if r is False: return False
    if _sqlite_claim(client_id): _SENT_CLIENT_IDS.add(client_id); return True
    return False
...
def _order_matches(o:dict, coin:str, is_buy:bool, limit_px:float, size:float)->bool:
    try:
        ...
        if isinstance(ib,str): ib=ib.lower() in ("true","1","yes","buy","long")
        return (oc or "").upper()==coin.upper() and bool(ib)==bool(is_buy) and abs(px-float(limit_px))<=1e-12 and abs(sz-float(size))<=1e-9
    except: return False

def submit_signal(sig)->None:
    if sig is None: raise ValueError("submit_signal(sig): sig is None")
    entry_low=getattr(sig,"entry_low",None); entry_high=getattr(sig,"entry_high",None)
    if entry_low is None or entry_high is None: raise ValueError("Signal missing entry_band=(low, high).")
    symbol=(getattr(sig,"symbol","") or "")
    if not _symbol_ok(symbol):
        log.info("[HL] SKIP: %s not in HYPER_ONLY_EXECUTE_SYMBOLS=%s", symbol, sorted(_ALLOWED)); return
    side_raw=(getattr(sig,"side","") or "").upper()
    if side_raw not in {"LONG","SHORT"}: raise ValueError(f"Unsupported side '{side_raw}'.")
    side="BUY" if side_raw=="LONG" else "SELL"

    coin=_coin_from_symbol(symbol)
    entry_low=float(entry_low); entry_high=float(entry_high); mid=(entry_low+entry_high)/2.0
    client_id=getattr(sig,"client_id",None)
    if not _claim_client_id(client_id):
        log.info("[HL] SKIP: client_id already processed: %s", client_id); return
    ...
    if tif_map.get("limit",{}).get("tif")=="Alo":
        if side=="BUY": limit_px=_quantize_down(max(price_tick, limit_px-price_tick), price_tick)
        else:           limit_px=_quantize_down(limit_px+price_tick, price_tick)
    ...
    if min_sz is not None and size < min_sz:
        log.info("[HL] SKIP: size %.10f < min %.10f for %s (raw=%.10f step=%g px=%.6f notional/qty=%s)",
                 size, min_sz, coin, raw_size, size_step, limit_px, "fixed" if _FIXED_QTY is not None else "notional"); return
    if size <= 0.0:
        log.info("[HL] SKIP: non-positive size %.10f (raw=%.10f step=%g coin=%s px=%.6f)", size, raw_size, size_step, coin, limit_px); return

    is_buy=(side=="BUY")
    try:
        for o in _iter_open_orders(info):
            if _order_matches(o, coin,is_buy,limit_px,size):
                log.info("[HL] SKIP: identical open order already exists on book: %s", o); return
    except Exception as e:
        log.warning("[HL] open-order duplicate check failed (continuing): %s", e)

    log.info("[HL] PLAN side=%s symbol=%s coin=%s band=(%.6f, %.6f) mid=%.6f pxTick=%g szStep=%g minSz=%s sz=%.10f SL=%s lev=%s TIF=%s client_id=%s",
             side, symbol, coin, entry_low, entry_high, mid, price_tick, size_step, "None" if min_sz is None else f"{min_sz}",
             size, getattr(sig,"stop_loss",None), getattr(sig,"leverage",None), tif_map, client_id)

    order={"coin":coin,"is_buy":is_buy,"sz":float(size),"limit_px":float(limit_px),
           "order_type":tif_map,"reduce_only":False,"client_id":client_id}
    ...
    try:
        ...
        try:
            if isinstance(resp,dict):
                ...
                if errs: log.warning("[HL] REJECT by exchange: %s", errs)
                elif isinstance(oks,list) and oks and isinstance(oks[0],dict):
                    s=oks[0].get("status") or oks[0].get("result") or oks[0].get("error")
                    if s and str(s).lower() not in ("ok","accepted","success","resting"):
