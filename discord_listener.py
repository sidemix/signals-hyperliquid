...
WATCH_CHANNEL_IDS = [
    int(x)
    for x in [s.strip() for s in (os.getenv("WATCH_CHANNEL_IDS", "") or "").split(",") if s.strip()]
    if str(x).isdigit()
]
...
_PROC_TAG = (
    ...
    or hex(abs(hash(os.getpid())) & 0xFFFFFFFF)[2:]
)
...
_redis = None
_REDIS_REQUIRED = bool(_IDEMP_REDIS_URL)
_REDIS_OK = False
...
def _redis_claim_msg(msg_id: str) -> Optional[bool]:
    """Try to claim a message ID in Redis."""
...
def _sqlite_conn():
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS processed_msgs (msg_id TEXT PRIMARY KEY, ts INTEGER NOT NULL)"
    )
    return conn
...
def _sqlite_claim_msg(msg_id: str) -> bool:
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO processed_msgs (msg_id, ts) VALUES (?, ?)", (msg_id, now)
        )
...
def claim_discord_message(msg_id: str) -> bool:
    """Ensure each message executes only once across pods."""
    if not msg_id:
        return True
    ...
    if _REDIS_REQUIRED:
        ...
        if r is False:
            return False
        log.warning(
            "[IDEMP] Redis configured but unavailable in this pod; SKIPPING message %s",
            msg_id,
        )
        return False
...
class Listener(discord.Client):
    ...
    async def on_ready(self):
        ...
        try:
            ...
            pass
        log.info(
            "[READY][proc=%s] Logged in as %s | watching=[%s]",
            _PROC_TAG,
            tag,
            watching,
        )
    ...
    async def on_message(self, message: discord.Message):
        try:
            ...
            log.info(
                "[RX][proc=%s] ch=%s by=%s id=%s len=%s",
                _PROC_TAG,
                message.channel.id,
            ...
            if not claim_discord_message(msg_id):
                log.info(
                    "[EXEC][proc=%s] SKIP: already processed (or Redis unavailable) message_id=%s",
                    _PROC_TAG,
                    msg_id,
                )
                return
            ...
            log.info(
                "[PARSER][proc=%s] side=%s symbol=%s band=(%s,%s) sl=%s lev=%s tif=%s client_id=%s",
                _PROC_TAG,
